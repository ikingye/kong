...
describe("Plugin: request-transformer (access)", function()
  ...
  describe("remove", function()
    ...
    it("parameters on multipart POST", function()
      ...
      assert.response(r).has.status(200)
      local json = assert.response(r).has.jsonbody()
      assert.is_nil(json.form.toremoveform)
      assert.equals("yes", json.form.nottoremove)
    end)
  ...
  describe("rename", function()
    ...
    it("parameters on multipart POST", function()
      ...
      assert.response(r).has.status(200)
      local json = assert.response(r).has.jsonbody()
      assert.is_nil(json.form["originalparam"])
      assert.is_not_nil(json.form["renamedparam"])
      assert.equals("yes", json.form["renamedparam"])
      assert.equals("yes", json.form["nottorename"])
    end)
  ...
  describe("replace", function()
    ...
    it("specified parameters on multipart POST", function()
      ...
      assert.response(r).has.status(200)
      local json = assert.response(r).has.jsonbody()
      assert.equals("v1", json.form.p1)
      assert.equals("v1", json.form.p2)
    end)
  ...
  describe("add", function()
    ...
    it("does not change or append value to parameter on multipart POST when parameter exists", function()
      ...
      assert.response(r).has.status(200)
      local json = assert.response(r).has.jsonbody()
      assert.equals("this should not change", json.form.p1)
      assert.equals("world", json.form.hello)
    end)
  ...
  describe("append ", function()
    ...
    it("new parameter in url encoded body on POST if it does not exist", function()
      ...
      assert.response(r).has.status(200)
      local json = assert.response(r).has.jsonbody()
      assert.are.same({"v1", "v2"}, json.form.p1)
      assert.are.same("value:1", json.form.p2)
    end)
    it("values to existing parameter in url encoded body if parameter already exist on POST", function()
      ...
      assert.response(r).has.status(200)
      local json = assert.response(r).has.jsonbody()
      assert.are.same({"v0", "v1", "v2"}, json.form.p1)
      assert.are.same("value:1", json.form.p2)
    end)
    it("does not fail if JSON body is malformed in POST", function()
      local r = assert(client:send {
    ...
    it("does not change or append value to parameter on multipart POST", function()
      ...
      assert.response(r).has.status(200)
      local json = assert.response(r).has.jsonbody()
      assert.are.same("This should not change", json.form.p1)
    end)
  end)
